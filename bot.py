...
def activate_user(user_id, days):
    ...
    conn = sqlite3.connect(DB_PATH)
    conn.execute("""
        UPDATE users SET is_active=1, joined_at=?, expires_at=? WHERE user_id=?
...
def get_expired_users():
    ...
    conn.row_factory = sqlite3.Row
    rows = conn.execute("""
        SELECT * FROM users WHERE is_active=1 AND expires_at < datetime('now')
    """).fetchall()
    conn.close()
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_active:
        ...
        txt = (
            ...
            f"💰 Narxi: *{stars} ⭐ Stars*\n"
            f"📅 Muddat: *{days} kun*\n\n"
            f"💳 Stars yoq? @tezstar_bot orqali\n"
            f"🇺🇿 UZS karta bilan sotib oling!"
        )
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main":
        ...
        await q.edit_message_text(txt, parse_mode="Markdown", reply_markup=main_kb(u.id))
        ...
        await q.edit_message_text(
            f"✅ *Obunangiz faol!*\n\n"
            f"📅 Tugash sanasi: *{user['expires_at'][:10]}*\n"
        ...
        await q.edit_message_text(
            "❓ *Yordam*\n\n"
            "1️⃣ *Obuna bolish* tugmasini bosing\n"
            "2️⃣ Stars yoq? @tezstar_bot dan sotib oling\n"
            "3️⃣ Stars bilan tolov qiling\n"
            "4️⃣ Kanalga avtomatik qoshilasiz\n\n"
        ...
        await q.edit_message_text(
            ...
            f"📢 Kanal nomi: *{s.get('channel_name')}*\n\n"
            f"Ozgartirish uchun:\n"
            f"`/setprice 150` — Stars narxi\n"
            f"`/setdays 30` — Muddat (kun)\n"
            f"`/setname Kanal nomi` — Kanal nomi\n"
            f"`/setmsg Xabar matni` — Xush kelibsiz xabari",
...
async def set_price(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Stars narxi *{ctx.args[0]} ⭐* ga ozgartirildi!", parse_mode="Markdown")
    except Exception:
...
async def set_days(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ Muddat *{ctx.args[0]} kun* ga ozgartirildi!", parse_mode="Markdown")
    except Exception:
...
async def set_name(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    val = " ".join(ctx.args)
    if not val:
        await update.message.reply_text("❌ Format: `/setname Kanal nomi`", parse_mode="Markdown"); return
    set_setting("channel_name", val)
    await update.message.reply_text(f"✅ Kanal nomi *{val}* ga ozgartirildi!", parse_mode="Markdown")

async def set_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.id not in ADMIN_IDS: return
    val = " ".join(ctx.args)
    if not val:
        await update.message.reply_text("❌ Format: `/setmsg Xabar matni`", parse_mode="Markdown"); return
...
async def add_user(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        link = await ctx.bot.create_chat_invite_link(
            ...
            expire_date=datetime.utcnow() + timedelta(hours=24))
        await update.message.reply_text(
            f"✅ `{uid}` qoshildi!\nHavola: {link.invite_link}", parse_mode="Markdown")
    except Exception as e:
...
async def remove_user(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(f"✅ `{uid}` chiqarildi!", parse_mode="Markdown")
    except Exception as e:
                ...
                for u in get_expired_users():
                    try:
                        ...
                        await app.bot.send_message(
                            ...
                            reply_markup=InlineKeyboardMarkup([
                                [InlineKeyboardButton("⭐ Obuna bolish", callback_data="subscribe")]]))
                        logger.info(f"User {u['user_id']} chiqarildi")
                    except Exception as e:
